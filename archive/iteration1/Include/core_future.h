...
namespace scp { namespace core {
	...
	struct awaitable_state : public awaitable_state_base
	{
		...
		//awaitable_state(T&& value) : awaitable_state_base(), _value(value) {}
		//awaitable_state() : awaitable_state_base(), _value() {}
...
// TODO - Create a new version (via mutually exclusive #ifdef pair)
// which has no embedded coro support. Put it all into
// traits, similar to below, but with all the methods embedded in
// promise_type, e.g. get_return_object()...
// This *should* separate the construction paths.

// See VS 2017 <future> implementation for a split implementation
