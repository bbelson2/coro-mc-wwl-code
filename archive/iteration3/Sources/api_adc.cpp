...
int findAdcIndex(uint8_t channelId) {
	for (int index = 0; index < sizeof(adc_wrappers)/sizeof(adc_wrapper_t) - 1; index++) {
		if (adc_wrappers[index].channelId == channelId) {
			return index;
		}
	}
	return -1;
...
const adc_wrapper_t * findAdc(uint8_t channelId) {
	int index = findAdcIndex(channelId);
	return (index < 0) ? nullptr : (adc_wrappers + index);
}
...
static_promise_t<word> _read_adc4_promises[2];
future_t<word, static_ptr<word>> read_adc4(uint8_t channelId) {
	auto adcIndex = findAdcIndex(channelId);
	auto w = adc_wrappers + adcIndex;
	static_promise_t<word>& p = _read_adc4_promises[adcIndex];
	int i = 0;
	split_phase_event_t::reg(w->measureEventId,
		[w]() { w->measure(false); },
		[w, s = p._state]() {
